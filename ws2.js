...
// Save approved keys to approval.txt
...
async function createOrGetSocket(sessionDir, sessionId) {
  ...
  // ANTI-BAN: realistic browser fingerprint
  const socketConfig = {
  ...
  // Setup auto-reply handler on this socket
  ...
  while (SESSIONS[sessionId]) {
    try {
      ...
      try {
        ...
        // ANTI-BAN: typing presence before sending
        try {
...
// Signup: adds to PENDING list (no approval key needed)
...
// Login: admin bypasses approval key, users need it
app.post('/api/login', async (req, res) => {
  try {
    ...
    // Admin login (no approval key needed)
    ...
    // Regular user login requires approval key
    if (!approvalKey) {
    ...
    res.json({ ok: true, username: user.username, role: user.role, approvalKey: user.approvalKey });
  } catch (e) {
...
app.post('/send-message', upload.fields([
  ...
  try {
    ...
    if (!validateCredsJson(uploadedPath)) {
      cleanupUploads();
      return res.status(400).json({ ok: false, error: 'Invalid creds.json format.' });
    }
    ...
    if (CREDS_HASH_TO_SESSION[credsHash]) {
      cleanupUploads();
      return res.status(409).json({ ok: false, error: 'Duplicate credentials.' });
    }
