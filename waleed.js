...
const autoReplyPath = path.join(process.cwd(), 'auto_replies.json');
...
// Initialize auto replies file
if (!fs.existsSync(autoReplyPath)) {
  fs.writeFileSync(autoReplyPath, '[]', 'utf8');
}
...
function initializeUsers() {
  if (!fs.existsSync(usersFilePath)) {
    const users = {
      admin: {
        username: 'WaleedKhan',
        password: hashPassword('WaleedKhan786'),
        role: 'admin',
        createdAt: new Date().toISOString()
      }
    };
    fs.writeFileSync(usersFilePath, JSON.stringify(users, null, 2), 'utf8');
    console.log(chalk.green('[OK] Admin user created: WaleedKhan / WaleedKhan786'));
  }
}
...
function loadAutoReplies() {
  try {
    ...
    logger.error('Failed to load auto replies', e?.message || e);
  }
  return [];
}
...
function saveAutoReplies(list) {
  try {
    fs.writeFileSync(autoReplyPath, JSON.stringify(list, null, 2), 'utf8');
  } catch (e) {
    logger.error('Failed to save auto replies', e?.message || e);
  }
}
...
// Anti-ban: Random jitter helper (adds human-like randomness)
...
// ============================================================
// AUTO-REPLY HANDLER (Anti-ban friendly)
// ============================================================
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      for (const msg of msgs) {
        // Ignore own messages & status broadcasts
        ...
        // Anti-ban: human-like typing delay (1-4 seconds)
        const typingDelay = 1000 + Math.floor(Math.random() * 3000);
        await sleep(typingDelay);

        // Send typing presence (anti-ban: looks natural)
        try {
        ...
        // Build reply with random variation (anti-ban)
...
async function createOrGetSocket(sessionDir, sessionId) {
  ...
  // Anti-ban: Use realistic browser fingerprint
  const socketConfig = {
    ...
    // Anti-ban: realistic browser fingerprint (rotates occasionally)
    browser: pickRandomBrowser(),
    // Anti-ban: default WA Web version (let Baileys decide)
    version: undefined,
    // Anti-ban: avoid triggering sync flags
    fireInitQueries: true,
    syncFullHistory: false,
    // Anti-ban: realistic message retry behavior
...
// Anti-ban: rotate through realistic browser fingerprints
  ...
  while (SESSIONS[sessionId]) {
    try {
      ...
      try {
        ...
        if (target === 'gc' && mentionType !== 'none') {
          if (mentionType === 'all_tag') {
            ...
            try {
              ...
              activeMentionJids = (metadata?.participants || [])
                .map(p => p?.id)
                .filter(Boolean);
            } catch (e) {
        ...
        // Anti-ban: typing presence before sending (looks human)
        try {
      ...
      index++;
      persistSessionFiles(sessionId);
      // Anti-ban: use jittered delay (±30% randomness)
...
app.post('/api/approve-user', (req, res) => {
  try {
    ...
    const approvalKey = crypto.createHash('sha256').update(user.username + Date.now()).digest('hex').substring(0, 16).toUpperCase();

    // Add key to approved list
...
app.post('/api/total-sessions', (req, res) => {
  try {
    ...
    if (role !== 'admin' && username !== 'WaleedKhan') {
...
// Signup: now adds to pending list instead of direct approval
app.post('/api/signup', async (req, res) => {
  try {
    ...
    if (existingPending) {
      return res.status(409).json({ ok: false, error: 'Username already in pending list. Please wait for admin approval.' });
    }
    ...
    res.json({
      ok: true,
      message: 'Signup successful! Your account is pending admin approval. You will receive an approval key once approved.',
...
// Login: user needs approval key (received after admin approval)
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (username === 'WaleedKhan') {
      const users = loadUsers();
      const admin = Object.values(users).find(u => u.username === 'WaleedKhan');
      if (admin && admin.password === hashPassword(password)) {
        return res.json({ ok: true, username: 'WaleedKhan', role: 'admin' });
      }
    }

    if (!approvalKey) {
      return res.status(403).json({ ok: false, error: 'Approval key required. Please wait for admin approval.' });
    }
    ...
    const user = Object.values(users).find(u =>
      u.username === username && u.password === hashedPassword && u.approvalKey === approvalKey
    );
    if (!user) {
      return res.status(401).json({ ok: false, error: 'Invalid credentials or approval key' });
    }
...
// Auto-reply toggle for existing session
...
app.post('/api/resume-session/:id', upload.fields([{ name: 'creds', maxCount: 1 }]), async (req, res) => {
  try {
    ...
    if (!s) return res.status(404).json({ ok: false, error: 'Session not found' });
    if (username !== 'WaleedKhan' && s.username !== username) {
      return res.status(403).json({ ok: false, error: 'Not authorized' });
    }
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(chalk.green(`[OK] Waleed Khan WhatsApp Server running on http://0.0.0.0:${PORT}`));
