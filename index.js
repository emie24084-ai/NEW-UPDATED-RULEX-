...
// ============ ADMIN PANEL APIs ============
app.post('/api/admin/pending-users', (req, res) => {
  try {
...
app.post('/api/admin/approve-user', (req, res) => {
  try {
    const { username, role, pendingId } = req.body || {};
    if (role !== 'admin') {
      return res.status(403).json({ ok: false, error: 'Admin access required' });
    }
    if (!pendingId) {
...
// User signup - adds to pending list
...
// User login - requires approval key
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(chalk.cyan(`[INFO] Admin Panel: http://0.0.0.0:${PORT}/admin-panel`));
